	...
	function ban_bro($reason) {	
		...
		if($result = $uConnect->query("SELECT * FROM `AC_Bans` WHERE (`HardwareID` = '" . $hardwareid . "' OR `HardwareID2` = '" . $hardwareid2 . "' OR `Volume` = '" . $volume . "' OR `IP` = '" . $ip . "') AND `Timestamp` = 0")) {
			if($result->num_rows == 0) {
				$uConnect->query("INSERT INTO `AC_Bans`(`HardwareID`, `HardwareID2`, `HardwareID3`, `HardwareID4`, `Volume`, `IP`, `Name`, `Reason`) VALUES ('" . $hardwareid . "', '" . $hardwareid2 . "', '" . $hardwareid3 . "', '" . $hardwareid4 . "', '" . $volume . "', '" . $ip . "', '" . $name . "', '" . $realreason . "')");
			}
		} else {
			$uConnect->query("INSERT INTO `AC_Bans`(`HardwareID`, `HardwareID2`, `HardwareID3`, `HardwareID4`, `Volume`, `IP`, `Name`, `Reason`) VALUES ('" . $hardwareid . "', '" . $hardwareid2 . "', '" . $hardwareid3 . "', '" . $hardwareid4 . "', '" . $volume . "', '" . $ip . "', '" . $name . "', '" . $realreason . "')");
		}
